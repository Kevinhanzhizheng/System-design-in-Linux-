...
int quit()
{
    ...
    int a=0, b = 0;
    char *buf = "";
    for(int a =0; pathname[a]; a++){
      char path1[a];
      strcpy(pathname, buf);
      strcat(pathname,"/");
      strcat(pathname,root->name);
      fprintf(file,root->type,pathname);
      while(b < 5){
        fprintf(file,root->sibling->type,pathname);
        root->sibling = root->sibling->sibling;
        b++;
      }
      if(root->child){
        fprintf(file,root->child->type,pathname);
      }
